...
type TestCase struct {
	...
	RawBody           []byte           `json:"-"`                              // when we have a resposne JWT, none text body that needs converting
	...
	SigningAlg        string           `json:"signingAlg,omitempty"` // signingAlg to use for POST/PATCH if set - otherwise use requestObjectSigningAlg from context
	Runner            `json:"-"`
}

// Runner - interface to run testcase
type Runner interface {
	Run(tc *TestCase, ctx *Context) (*resty.Response, error)
}
...
// // Run - pass the test to a TestRunner to execute
// func (t *TestCase) Run(ctx *Context) error {
// 	if t.TestRunner != nil {
// 		t.Run()

// 		return t.TestRunner(ctx)
// 	}
// 	return nil
// }
...
func (t *TestCase) Validate(resp *resty.Response, ctx *Context) (bool, []error) {
	...
	// if contentType is applicationJWT convert the body.
	// if applicationJWT signature checking is enabled.
	//    get the signingAlg, find the appropriate keys, check the signature
	if hasContentTypeJWT(resp.Header()) {
		logrus.Debug("  We have a jwt - lets crack it open")
		payload, err := auth.JSONfromJWT(string(t.RawBody))
		if err != nil {
			return false, []error{errors.New("Validate: failed to extract JWT payload: " + err.Error())}
		}
		_ = payload
	}
...
func hasContentTypeJWT(headers http.Header) bool {
	contentType := headers.Get("content-type")
	if strings.ToLower(contentType) == "application/jwt" {
		return true
	}
	return false
}
...
func validateSignature(signature, body string, ctx *Context) (bool, error) {
	...
	if signature != "" {
		...
		b64encoding, err := auth.GetB64Encoding(ctx)
		if err != nil {
			return false, errors.New("ValidationSignature cannot get B64Encoding: " + err.Error())
		}

		pass, err = auth.ValidateSignature(signature, body, jwksURI, b64encoding)
		if err != nil {
...
func (t *TestCase) Clone() TestCase {
	...
	tc.Bearer = t.Bearer
	tc.ValidateSignature = t.ValidateSignature

	tc.Input = t.Input.Clone()
	tc.Context = t.Context.Clone()
	tc.Expect = t.Expect.Clone()
	for _, v := range t.ExpectOneOf {
		tc.ExpectOneOf = append(tc.ExpectOneOf, v.Clone())
	}
...
// CreateChild tests -
func (t *TestCase) CreateChild() TestCase {
	tc := TestCase{}

	tc.ValidateSignature = t.ValidateSignature
	tc.Bearer = t.Bearer
	tc.DoNotCallEndpoint = t.DoNotCallEndpoint
	tc.APIName = t.APIName
	tc.APIVersion = t.APIName
	...
	// Fix X_FCS_TestCase Header TODO
...
func (t *TestCase) ProcessReplacementFields(ctx *Context, showReplacementErrors bool) {
	...
	for k, v := range t.Expect.ContextPut.Matches {
		...
		if err != nil {
			...
			t.logReplaceError("ContextName", err, logger, false)
		}
	}
...
func (t *TestCase) logReplaceError(field string, err error, logger *logrus.Logger, showReplacementErrors bool) {
	if showReplacementErrors {
		logrus.Warnf("Testcase: processing %s replacement fields %s", field, err.Error())
	} else {
		logrus.Warnf("Testcase: processing %s replacement fields", field)
	}
}
...
func (t *TestCase) processReplacementFormData(ctx *Context) {
	...
	for k := range t.Input.FormData {
		...
		if err != nil {
			logrus.StandardLogger().WithError(err).Warn("processing replacement fields")
		}
	}
}
