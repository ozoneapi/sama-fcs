...
func (i *Input) CreateRequest(tc *TestCase, ctx *Context) (*resty.Request, error) {
	...
	// certificates?
	// find certs in Context?
	client := httpclient.NewClient()
	req := client.R() // create basic request that will be sent to endpoint
	...
	if len(i.RequestBody) > 0 { // set any input raw request body ("bodyData")
		body, err := i.getBody(req, ctx, tc.SigningAlg)
		if err != nil {
			return nil, err
		}
...
func (i *Input) setClaims(tc *TestCase, ctx *Context) error {
	...
	if len(i.Claims) > 0 { // create JWT from claims - put in context?
		...
		case "parJwt":
			logrus.Info("PAR Consent JWT Generation")
			token, err := i.GenerateParToken(ctx)
			if err != nil {

			}
			ctx.PutString("par_jwt", token)
			parEndpoint, _ := ctx.GetString("par_authorization_endpoint")
			tc.Input.Endpoint = parEndpoint       // Result - set jwt token in endpoint url
			ctx.PutString("par_url", parEndpoint) // make consent available in context
			logrus.Tracef("==> parURL: %s", parEndpoint)

		case "parConsentUrl":
			authEndpoint, _ := ctx.Get("authorisation_endpoint")
			consent := parConsentURL(authEndpoint.(string), i.Claims)
			ctx.PutString("consent_url", consent)
			tc.Input.Endpoint = i.Claims["aud"] + "/PsuDummyURL"
			tc.DoNotCallEndpoint = true
		case "parConsentUrlHeadless":
			authEndpoint, _ := ctx.Get("authorisation_endpoint")
			consent := parConsentURL(authEndpoint.(string), i.Claims)
			ctx.PutString("consent_url", consent)
		...
		case "consenturl":
			...
			logrus.Tracef("===> consentURL: %s", consent)
...
// GenerateParToken -
func (i *Input) GenerateParToken(ctx *Context) (string, error) {
	alg, err := ctx.GetString("requestObjectSigningAlg")
	if err != nil && err != ErrNotFound {
		return "", err
	}
	signingMethod, err := auth.GetSigningAlg(alg)
	if err != nil {
		logrus.Warnln("Using Unsigned Par Jwt as Request Object")
		return i.generateUnsignedParJWT(ctx) // not sure if this is still appropraite
	}
	return i.generateParJWT(ctx, signingMethod)
}
...
func parConsentURL(authEndpoint string, claims map[string]string) string {
	...
	queryString.Set("request_uri", claims["request_uri"])
...
func consentURL(authEndpoint string, claims map[string]string, token string) string {
	...
	queryString.Set("nbf", claims["nbf"])
...
func (i *Input) createApplicationJwt(signingAlg string, ctx *Context) (string, error) {
	if len(i.RequestBody) == 0 {
		//return "", errors.New("cannot create application/JWT with an empty request body")
		i.RequestBody = "{}" // Allows for tests with valid empty body
	}
	jwt, err := auth.CreateSignedJWT(signingAlg, i.RequestBody, i.Claims, ctx)
	if err != nil {
		return "", errors.Wrap(err, "CreateApplicationJwtBody failed")
	}
	logrus.Debugf("application/jwt token: %s", jwt)
	return jwt, nil

}

func (i *Input) createJWSDetachedSignature(ctx auth.ContextInterface) error {
	if len(i.RequestBody) == 0 {
...
// retrieve body
// process any replacment fields in body
// if contentType contains "application/json"
//
//	minify the body
func (i *Input) getBody(req *resty.Request, ctx *Context, signingAlg string) (string, error) {
	value := i.RequestBody
	var err error
	for {
	...
	i.RequestBody = value
	body, err := i.setBodyFromContentType(signingAlg, ctx)
	if err != nil {
		return "", fmt.Errorf("Error setting ContentType " + err.Error())
	}
	i.RequestBody = body
	req.SetBody(body)
	return body, nil
...
// Allows direct setting of content type
func (i *Input) SetContentTypeHeader(ct string) {
	for key := range i.Headers { // ensure just one content-type to rule them all
		if strings.ToLower(key) == "content-type" {
			delete(i.Headers, key)
		}
	}
	i.SetHeader("Content-Type", ct)
}

func (i *Input) setBodyFromContentType(signingAlg string, ctx *Context) (string, error) {
	var body string
	var err error
	value := i.RequestBody
	contentType := i.contentTypeHeader()

	if strings.Contains(contentType, "application/json") || strings.Contains(contentType, "application/jwt") {
		m := minify.New()
	...
	if strings.Contains(contentType, "application/jwt") {
		if len(signingAlg) == 0 {
			signingAlg, _ = ctx.GetString("requestObjectSigningAlg")
		}
		if i.Method == "POST" || i.Method == "PATCH" {
			body, err = i.createApplicationJwt(signingAlg, ctx)
			if err != nil {
				logrus.Tracef("error creating applicationJWT: %s", err)
				return "", err
			}
			i.RequestBody = body
		} else {
			return "", errors.New("can only apply application/JWT to POST and PATCH, not " + i.Method + " methods")
		}
	}
	return i.RequestBody, nil
...
// CreateLtgConsentJwt - Creates letter of Guarantee Consent Jwt
func (i *Input) CreateLtgConsentJwt(ctx *Context) (string, error) {
	uuid := uuid.New()
...
func signingCertFromContext(ctx *Context) (auth.Certificate, error) {
	...
	cert, err := auth.NewCertificate(pubKey, privKey)
	if err != nil {
...
func (i *Input) generateParJWT(ctx *Context, alg jwt.SigningMethod) (string, error) {
	uuid := uuid.New()
	claims := jwt.MapClaims{}
	...
	claims["code_challenge"] = "testchallenge"
...
func (i *Input) generateUnsignedParJWT(ctx *Context) (string, error) {
	logrus.Warn("generating unsigned ParJWT")
	uuid := uuid.New()
	claims := jwt.MapClaims{}
	...
	responseType, err := ctx.GetString("responseType")
	if err != nil {
		logrus.Warnf("cannot find responseType in context %w", err)
	}
	claims["response_type"] = responseType
	claims["nbf"] = time.Now().Unix()
	claims["exp"] = time.Now().Add(time.Minute * time.Duration(3)).Unix()
	claims["nonce"] = uuid
...
func PrintJwt(tokenString string) error {
	...
	if err != nil {
		logrus.Warn("DecodeJwt malformed token: %w", err)
	}
...
// PrintYAML - print as Yaml
func PrintYAML(j interface{}) error {
	var out []byte
	var err error
