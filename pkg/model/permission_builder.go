...
import "bitbucket.org/ozoneapi/sama-conformance-suite/pkg/permissions"
...
func (b permissionBuilder) includedPermission(ctx Context, endpoint string) permissions.PermissionSet {
	...
	if err != nil {
		return permissions.NoPermissionSet()
	}
	codes := []string{}
...
func (b permissionBuilder) excludedPermissions(ctx Context) permissions.PermissionSet {
...
func mapStringToCodeSet(values []string) permissions.PermissionSet {
	var result permissions.PermissionSet
	for _, value := range values {
		result = append(result, permissions.Permission(value))
	}
	return result
}
