...
func getPaymentConsents(definition RunDefinition, requiredTokens []testlist.RequiredTokens, ctx *model.Context) (ConsentTokens, error) {
	executor := &Executor{}
	...
	consentItems := make([]ConsentToken, 0)
	for _, rt := range requiredTokens {
		tci := ConsentToken{TokenName: rt.Name, ConsentURL: rt.ConsentURL, ConsentID: rt.ConsentID}
...
func runPaymentConsents(rt []testlist.RequiredTokens, ctx *model.Context, executor *Executor) ([]testlist.RequiredTokens, error) {
...
func (r *TestCaseRunner) executePaymentConsent(tc model.TestCase, ruleCtx *model.Context, log *logrus.Entry) (bool, []string) {
	testresult := r.executeTest(tc, ruleCtx)
