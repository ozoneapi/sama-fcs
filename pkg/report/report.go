...
import (
	...
	internal_time "bitbucket.org/ozoneapi/sama-conformance-suite/pkg/time"
...
type Report struct {
	...
	Discovery        discovery.Template `json:"-"`                        // Original used discovery model
...
func (r *Report) manifestFilePaths() []string {
	paths := make([]string, 0, len(r.Discovery.APICollection.APIDefinitions))
	for _, manifest := range r.Discovery.APICollection.APIDefinitions {
		path := strings.TrimPrefix(manifest.Tests, "file://")
