...
import (
	...
	"bitbucket.org/ozoneapi/sama-conformance-suite/pkg/version"
...
func GetImplementedTestCases(disco *discovery.APIDefinition, nameGenerator names.Generator, ctx *model.Context, headlessTokenAcquisition bool, genConfig GeneratorConfig) ([]model.TestCase, map[string]string) {
...
func GetCustomTestCases(discoReader *discovery.CustomTest, ctx *model.Context, headlessTokenAcquisition bool) APITests {
	spec := discovery.APISpecification{Name: discoReader.Name}
	specTestCases := APITests{APISpec: spec}
	testcases := []model.TestCase{}
...
func getResourceIds(item *discovery.APIDefinition, path string, genConfig GeneratorConfig) string {
	result := path
