...
// LoadTestDefinitions loads the scripts from JSON encoded contents of filename
// and returns Scripts objects
func LoadTestDefinitions(filename string) (TestDefinitions, error) {
	if strings.HasPrefix(filename, "https://") {
		return TestDefinitions{}, errors.New("https:// manifest loading not implemented")
	}
...
//		]
//	}
func MapDiscoveryEndpointsToManifestTestIDs(disco *discovery.Template, mf TestDefinitions) DiscoveryPathsTestIDs {
	mapURLTests := make(DiscoveryPathsTestIDs)

	// Iterate the discoveryModel.discoveryItems.endpoints
	for _, discoItem := range disco.APICollection.APIDefinitions {
		for _, discoEndpoint := range discoItem.Endpoints {
			...
			for _, mfScript := range mf.TestDefinitions {
...
func FindUnmatchedManifestTests(mf TestDefinitions, mappedTests DiscoveryPathsTestIDs) []string {
	var result []string
	for _, script := range mf.TestDefinitions {
		i := 0
