...
// NewPrivateKey - Wrap Private Key in 'legacy' cert wrapper ... check use in signing
func NewPrivateKey(privateKeyPem string) (Certificate, error) {
...
// NewPrivateCertificate -
// Signing just requires Private Signing Key - no pub key required
func NewPrivateCertificate(privateKeyPem string) (Certificate, error) {
	privateKey, err := jwt.ParseRSAPrivateKeyFromPEM([]byte(privateKeyPem))
	if err != nil {
		return nil, fmt.Errorf("error with private key: %w", err)
	}
