...
type Validator interface {
	Validate(*Template) (ValidationFailures, error)
}
...
type funcWrapperValidator struct {
	validatorFunc         func(checker model.ConditionalityChecker, discovery *Template) (bool, []ValidationFailure, error)
	conditionalityChecker model.ConditionalityChecker
}

func (v funcWrapperValidator) Validate(model *Template) (ValidationFailures, error) {
